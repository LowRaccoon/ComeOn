package net.lowis.tutorialmod.enchantment.custom;
...
public record LightningStrikerEnchantmentEffect() implements EnchantmentEntityEffect {
    ...
    public void apply(ServerWorld world, int level, EnchantmentEffectContext context, Entity user, Vec3d pos) {
        switch (level) {
            case 1: EntityType.LIGHTNING_BOLT.spawn(world, user.getBlockPos(), SpawnReason.TRIGGERED);
            break;
            case 2: EntityType.LIGHTNING_BOLT.spawn(world, user.getBlockPos(), SpawnReason.TRIGGERED);
